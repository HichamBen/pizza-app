...
export default function PaymentS() {
  ...
  useEffect(() => {
    async function convertCurrency() {
      try {
        const { data } = await axios.get(
          `https://api.apilayer.com/currency_data/convert?to=USD&from=MAD&amount=${order.totalePrice}&apikey=evByCrrFufg7sC2ic4kc3s2RFbYZ8xKs`
        );
    ...
    async function createScript() {
      try {
        const { data } = await axiosPrivate.get("/api/config/paypalKey");
        ...
        .Buttons({
          ...
          onApprove: function (data, actions) {
            ...
            return actions.order.capture().then(function (details) {
              ...
              dispatch(registerOrder(axiosPrivate, "paypal"));
            });
          },
        })
  ...
  function finishOrder() {
    dispatch(registerOrder(axiosPrivate, "Upon receipt"));
  }
  ...
  return (
    <>
      ...
      {order.isPay || order.paymentMethod === "Upon receipt" ? (
        <Navigate to="/order/bill" />
      ) : (
        <div className="container mt-5" style={{ height: "55vh" }}>
          ...
          <div className="mx-auto px-3 py-2 mt-5" style={{ width: "375px" }}>
            <div className="row justify-content-sm-between justify-content-center">
              <div
                className="col-7 col-sm-5 paymentM-1 mb-4"
                ref={firstM}
                onClick={activeM}
              >
              ...
              <div
                className="col-7 col-sm-5 paymentM-2  mb-4"
                ref={secondM}
                onClick={activeM}
              >
