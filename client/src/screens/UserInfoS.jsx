...
import { addUserData, getUserInfo } from "../reduxjs/reducers/userSlice";
...
export default function UserInfoS() {
  ...
  const [name, setName] = useState(jwt_decode(token).username);
  const [lastN, setLastN] = useState("");
  const [address, setAddress] = useState("");
  const [phone, setPhone] = useState("");

  const [position, setPosition] = useState({
    lat: 30.42117447120587,
    lng: -9.60396195287973,
  });
  const [isLocationSpecify, setIsSpecify] = useState(false);
  const navigate = useNavigate();
  const { restLocation, clientLocation } = useSelector(
  ...
  useEffect(() => {
    ...
    async function createScript() {
      try {
        const { data } = await axiosPrivate.get("/api/config/googleKey");
  ...
  const handleSubmit = async e => {
    ...
    if (
      clientLocation.lat === 30.42117447120587 &&
      clientLocation.lng === -9.60396195287973
    ) {
        ...
        <div className="col-8 col-md-6 mx-auto mb-3">
          {isLocationSpecify ? (
            <span
              ...
              style={{ backgroundColor: "rgb(129, 224, 171, 0.5)" }}
            >
              You 're location is specify
            </span>
          ) : (
            <span
              className="text-danger p-1"
              style={{ backgroundColor: "rgba(233, 158, 158,0.5)" }}
            >
              You must be allow this app to access your location
            </span>
          )}
          <br />
          <span className="btn btn-success mb-3" onClick={getLocation}>
            Click to define your location In map
          </span>
          <div id="map"></div>
        </div>
