...
export default function CartS() {
  ...
  const { restaurantSelected: resChoosed, deferring } = useSelector(
  ...
  useEffect(() => {
    ...
    if (!resChoosed) {
      ...
      setSearchParms(qParms, { replace: true });
    }
  ...
  useEffect(() => {
    ...
    [...searchParms].forEach((query, id) => {
      if (searchParms.get("item_" + id) !== "0") {
        const amount =
          Number(searchParms.get("item_" + id)) / resChoosed.pizza[id].price;
        holdPizzaSelected = [
          ...holdPizzaSelected,
          {
            inUrlParams: id,
        ...
        <div className="container mt-5">
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title restaurant-brand">
                {resChoosed.restaurant}
              </h3>
            </div>
            <div className="modal-body">
              {resChoosed.pizza.map((item, id) => (
                <PizzaRow key={id} id={id} item={item} />
              ))}
              <div className="p-2 border fs-4 bg-light">
                <span>Total price:</span>
                <span className="float-end pe-5 text-success fw-bold">
                  {totalePrice}dh
                </span>
              </div>
            </div>
            <div className="modal-footer">
              {isAnItemThere && (
                <span className="text-danger">
                  You must choose an item to continue the order
                </span>
              )}
              ...
              <button className="btn btn-primary" onClick={isItemAdded}>
                Next
              </button>
            </div>
          </div>
